...
func TestPoller_AppliesDesiredState(t *testing.T) {
	...
	mux.HandleFunc("/v1/recording-servers/", func(w http.ResponseWriter, _ *http.Request) {
		...
		_ = json.NewEncoder(w).Encode(DesiredState{
			...
			VersionSet:        1,
			RecordingPolicies: []DesiredStateItem{
				mkItem("p1", "production", 1),
				mkItem("p2", "side", 1),
			},
		})
	})
...
func TestPoller_SkipsPreImport(t *testing.T) {
	// policy_canonical_source = recorder => poller should not actually GET.
	dir := t.TempDir()
	id, err := identity.Open(dir)
	require.NoError(t, err)
	...
	// Leave policy_canonical_source = recorder.
	require.NoError(t, id.SetPolicyCanonicalSource(identity.CanonicalSourceRecorder))
	...
	require.Equal(t, int32(0), atomic.LoadInt32(&requestCount),
		"pre-import recorder must not poll the MS for policies")
	mu.Lock()
	defer mu.Unlock()
...
func TestPoller_AckCarriesAppliedHighWaterMark(t *testing.T) {
	...
	server := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		// Echo the recorder id from the URL.
		urlPath := r.URL.Path
		const prefix = "/v1/recording-servers/"
		recID := ""
		if len(urlPath) > len(prefix)+36 {
			recID = urlPath[len(prefix) : len(prefix)+36]
		}
		_ = json.NewEncoder(w).Encode(DesiredState{
			RecordingServerID: recID,
			VersionSet:        5,
			RecordingPolicies: []DesiredStateItem{
				mkItem("p1", "production", 5),
			},
		})
	}))
	defer server.Close()
